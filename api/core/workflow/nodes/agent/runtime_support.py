...
from .exceptions import AgentInputTypeError, AgentParameterError, AgentVariableNotFoundError
from .strategy_protocols import ResolvedAgentStrategy


class AgentRuntimeSupport:
    @staticmethod
    def _normalize_variable_selector(value: Any) -> list[str] | None:
        if not isinstance(value, list):
            return None

        normalized_selector: list[str] = []
        for item in value:
            ...
            if not normalized_item:
                return None
            normalized_selector.append(normalized_item)
        ...
        *,
        ...
        for parameter_name in node_data.agent_parameters:
            ...
            match agent_input.type:
                case "variable":
                    variable_selector = self._normalize_variable_selector(agent_input.value)
                    if variable_selector is None:
                        if parameter.required:
                            raise AgentParameterError(
                                f"Parameter '{parameter_name}' requires a valid variable selector.",
                        ...
                        if variable is None:
                            if parameter.required:
                                raise AgentVariableNotFoundError(".".join(variable_selector))
                            parameter_value = None
                        else:
                        ...
                        for key, param in parameters.items():
                            if param.get("auto", ParamsAutoGenerated.OPEN) in (
                                ...
                                if value_param and value_param.get("type", "") == "variable":
                                    variable_selector = self._normalize_variable_selector(value_param.get("value"))
                                    if variable_selector is None:
                                        if param.get("required", False):
                                            raise AgentParameterError(
                                                f"Tool parameter '{key}' requires a valid variable selector.",
                                                parameter_name=parameter_name,
                                            )
                                        params[key] = None
                                    else:
